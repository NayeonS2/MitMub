...
for i in range(1, 50):
    request = (f'{HOST}{NOW_MOVIE_URL}?api_key={API_KEY}&language=ko&page={i}')
    response = urllib.request.urlopen(request)
    json_str = response.read().decode('utf-8')
    json_object = json.loads(json_str)
    ...
    for movie in data_movies:
        now_movie_Ids.append(movie.get("id"))

for idx, movie_Id in enumerate(now_movie_Ids):
    movie_request = (f'{HOST}{MOVIE_INFO_URI}{movie_Id}?api_key={API_KEY}&language=ko&')
    response = urllib.request.urlopen(movie_request)
    json_str = response.read().decode('utf-8')
    json_object = json.loads(json_str)
    if json_object.get("poster_path"):
        ...
        now_movie_list.append(my_object)




for i in range(1, 50):
    request = (f'{HOST}{UPCOMING_MOVIE_URL}?api_key={API_KEY}&language=ko&page={i}')
    response = urllib.request.urlopen(request)
    json_str = response.read().decode('utf-8')
    json_object = json.loads(json_str)
    ...
    for movie in data_movies:
        upcoming_movie_Ids.append(movie.get("id"))

for idx, movie_Id in enumerate(upcoming_movie_Ids):
    movie_request = (f'{HOST}{MOVIE_INFO_URI}{movie_Id}?api_key={API_KEY}&language=ko&')
    response = urllib.request.urlopen(movie_request)
    json_str = response.read().decode('utf-8')
    json_object = json.loads(json_str)
    if json_object.get("poster_path"):
        ...
        upcoming_movie_list.append(my_object)
...
with open('now_movies.json', 'w', encoding='UTF-8') as file:
    file.write(json.dumps(now_movie_list, ensure_ascii=False))
